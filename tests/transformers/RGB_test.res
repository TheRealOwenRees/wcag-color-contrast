...
describe("Flattened Alpha", () => {
  test("Black", () => {
    switch flattenAlpha({r: 1.0, g: 1.0, b: 1.0, a: 1.0}) {
    | {r: 1.0, g: 1.0, b: 1.0} => ()
    | _ => throw(Failure("Expected #ffffff to be HEX"))
    }
  })
  test("Black half alpha", () => {
    switch flattenAlpha({r: 1.0, g: 1.0, b: 1.0, a: 0.5}) {
    | {r: 1.0, g: 1.0, b: 1.0} => ()
    | _ => throw(Failure("Expected #ffffff to be HEX"))
    }
  })
  test("Black with green background reduced alpha", () => {
    switch flattenAlpha(
      ...
      ~backgroundColorObj={r: 0.0, g: 1.0, b: 0.0},
    ) {
    | {r: 0.8, g: 1.0, b: 0.8} => ()
    | _ => throw(Failure("Expected #ffffff to be HEX"))
    }
...
describe("Luminance", () => {
  test("Green", () => {
    let result = relativeLuminance({r: 0.6038, g: 1.0, b: 0.6038})
